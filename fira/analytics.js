...
class AnalyticsDashboard {
    ...
    createVelocityChart() {
        ...
        if (typeof Chart === 'undefined') {
            ...
            ctx.parentElement.innerHTML = '<p style="text-align: center; padding: 20px; color: #666;">Chart.js not available</p>';
            return;
        }
    ...
    createTimeBreakdownChart() {
        ...
        if (typeof Chart === 'undefined') {
            console.warn('Chart.js not loaded - skipping time breakdown chart');
            ctx.parentElement.innerHTML = '<p style="text-align: center; padding: 20px; color: #666;">Chart.js not available</p>';
            return;
        }
...
// Global initialization function for router
window.initializeAnalytics = function() {
    console.log('🎯 Initializing analytics dashboard');
    console.log('🎯 Analytics initialization called by router');
    if (!window.analyticsDashboard) {
        window.analyticsDashboard = new AnalyticsDashboard();
    } else {
        console.log('Analytics dashboard already initialized');
    }
    return Promise.resolve();
};

// Auto-initialize for standalone page
document.addEventListener('DOMContentLoaded', () => {
    // Only auto-initialize if we're not using router (standalone page)
